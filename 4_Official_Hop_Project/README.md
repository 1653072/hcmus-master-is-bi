...
- **LSET/CET + Lookback incremental (pull):** các pipeline load file luôn ghi toàn bộ file ngày hiện tại vào `raw_*`; chỉ Stream C của pipeline validate `raw_* → stg_*` mới lọc `event_ts >= (LSET - LookbackDays) AND event_ts <= CET`. `00_start` đọc `control.lookback_days` theo từng source và set JVM: `LOOKBACK_DAYS_DIVVY_TRIPS`, `LOOKBACK_DAYS_CITIBIKE_TRIPS`, `LOOKBACK_DAYS_NOAA` (mặc định seed = 3). Divvy/Citi lọc trên `started_at`; NOAA trên `observation_ts`. DQ Stream A/B vẫn đánh giá toàn bộ raw. GBFS station là full snapshot. Sau audit thành công, `control.lset`/`cet` tiến tới CET của lần chạy; Lookback không làm watermark lùi lại.
...
make db-up
make mdm-start            # scripts/start_mdm_station_services.sh (Hop -Xmx8g + push all stations)
...
# Free port 8080 nếu còn Hop cũ: MDM_KILL_EXISTING=1 make mdm-start
# Heap khác: HOP_OPTIONS="-Xmx6g" make mdm-start
```
...
| `HOP_OPTIONS`          | Heap JVM cho hop-server (tránh `Java heap space` khi push) | `-Xmx8g`                              |
| `MDM_KILL_EXISTING`    | Kill process đang listen port MDM rồi start lại            | unset (`1` = kill)                    |
